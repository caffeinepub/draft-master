...
import type { DraftData as _DraftData, UserProfile as _UserProfile, UserRole as _UserRole } from "./declarations/backend.did.d.ts";
export class Backend implements backendInterface {
    ...
    async generateShareLink(arg0: string): Promise<string> {
        if (this.processError) {
            try {
    ...
    async getDraftByShareLink(arg0: string): Promise<DraftData | null> {
        if (this.processError) {
            try {
                const result = await this.actor.getDraftByShareLink(arg0);
...
function from_candid_opt_n6(_uploadFile: (file: ExternalBlob) => Promise<Uint8Array>, _downloadFile: (file: Uint8Array) => Promise<ExternalBlob>, value: [] | [_DraftData]): DraftData | null {
    return value.length === 0 ? null : value[0];
}
